...
app.post('/analyze', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(`[${new Date().toISOString()}] Analyzing binary: ${req.file.originalname} (${req.file.size} bytes)`);
    ...
    console.log(`[${new Date().toISOString()}] Analysis complete: ${analysis.totalVulnerabilities} vulnerabilities found`);
...
app.post('/analyze/batch', upload.array('files', 10), async (req, res) => {
  try {
    ...
    console.log(`[${new Date().toISOString()}] Batch analyzing ${req.files.length} binaries`);
...
app.listen(PORT, () => {
  console.log('\n╔══════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                  ║');
  console.log('║             Mock ML Service for CypherRay SDK Testing           ║');
  console.log('║                                                                  ║');
  console.log('╚══════════════════════════════════════════════════════════════════╝\n');
