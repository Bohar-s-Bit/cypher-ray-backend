...
async function cleanupTestData() {
  try {
    ...
    const testUser = await usersCollection.findOne({
      $or: [{ email: TEST_USER_EMAIL }, { username: TEST_USER_USERNAME }],
    });
    ...
    const apiKeysResult = await apiKeysCollection.deleteMany({
      userId: testUser._id,
    });
    ...
    const jobsResult = await analysisJobsCollection.deleteMany({
      userId: testUser._id,
    });
    ...
    const transactionsResult = await creditTransactionsCollection.deleteMany({
      userId: testUser._id,
    });
    console.log(
      `✓ Deleted ${transactionsResult.deletedCount} credit transaction(s)`
    );
